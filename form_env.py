...
from  env_utils import Agent, Pose
...
class FormEnv(gym.Env):
    ...
    def configure_defaults(self):
        ...
        self.nbhrs = {j: [i for i in self.agents if i.id != j.id] 
    ...
    def render(self, mode='human'):
        ...
        for agent, agent_tf in zip(self.agents, self.agent_tfs):
            agent_tf.set_translation(*(agent.pose.asPoint() +
                                     self.w_limits//2)*self.scale)
            agent_tf.set_rotation(agent.pose.t)
        centroid = np.mean([a.pose.asPoint() for a in self.agents], 0)
    ...
    def compute_reward2(self):
        ...
        sides = sorted([norm(np.array(i.pose.asPoint()) - j.pose.asPoint()) 
                        for i in self.agents 
        ...
        return np.array([reward]), np.array([done]), {"success": done}
